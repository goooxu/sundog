...
**sundog** 是面向 NVIDIA GPU 的开源路径追踪渲染器，基于
**OptiX 9.1 + CUDA 13.0 + PhysX 5.8**。不假设特定 CPU 架构与 GPU 型号
（x86_64 与 aarch64 均可）：设备代码编为 compute_75 PTX 由驱动 JIT，
Turing 起的任何 NVIDIA GPU 都能跑，RT Core 非必需——没有 RT Core 的
GPU（如数据中心 Blackwell）由 OptiX 自动改走软件 BVH 遍历，结果不变；
物理场景要求 PhysX GPU 刚体（同为 sm_75 起，无 CPU 回退）。
**场景即程序**：每个场景是一个 Python 文件，直接运行它就完成
渲染——megakernel 路径追踪（NEE + MIS）、硬件三角形与大规模实例化、
PhysX GPU 刚体装载、OptiX AI 降噪，固定种子时输出逐位可复现。

![Assembly Hall](docs/gallery/15-assembly-hall.avif)
...
![Atelier](docs/gallery/16-atelier.avif)
...
![Dusk Tide](docs/gallery/17-dusk-tide.avif)
...
| 开 | 关 |
|:---:|:---:|
| ![on](docs/gallery/compare/transparent-shadows-on.avif) | ![off](docs/gallery/compare/transparent-shadows-off.avif) |
...
| 开 | 关 |
|:---:|:---:|
| ![on](docs/gallery/compare/flame-shadow-on.avif) | ![off](docs/gallery/compare/flame-shadow-off.avif) |
...
| 开 | 关 |
|:---:|:---:|
| ![on](docs/gallery/compare/env-importance-on.avif) | ![off](docs/gallery/compare/env-importance-off.avif) |
...
| 开 | 关 |
|:---:|:---:|
| ![on](docs/gallery/compare/mesh-light-on.avif) | ![off](docs/gallery/compare/mesh-light-off.avif) |
...
| 开 | 关 |
|:---:|:---:|
| ![on](docs/gallery/compare/frosted-glass-on.avif) | ![off](docs/gallery/compare/frosted-glass-off.avif) |
...
| 开 | 关 |
|:---:|:---:|
| ![on](docs/gallery/compare/nee-on.avif) | ![off](docs/gallery/compare/nee-off.avif) |
...
| 开 | 关 |
|:---:|:---:|
| ![on](docs/gallery/compare/ai-denoise-on.avif) | ![off](docs/gallery/compare/ai-denoise-off.avif) |
...
- **纹理**：solid / checker / grid / AVIF 图像（sRGB，无损）
...
- **物理装载**：场景声明刚体初始条件（`s.physics(…)` + 逐对象 opt-in），
  加载时用 **PhysX 5 GPU 刚体**（`eENABLE_GPU_DYNAMICS` + GPU 宽相，与渲染同卡模拟）
...
- **HDR 输出**：唯一输出格式为 12-bit PQ（SMPTE ST 2084）BT.2020 无损
  AVIF——线性 1.0 锚定 203 nits，高光原样保留交给 HDR 显示器，无 SDR
  色调映射路径；AOV 走 sRGB 8-bit 无损 AVIF
...
```
python3 scenes/07-campfire.py                    # 渲出 07-campfire.avif
