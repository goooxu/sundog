...
深夜变体（Sparky 的发光屏是唯一光源），同 256 spp：开——发光网格被
NEE 主动采样，屏光照明干净；关——只能被 BSDF 路径偶然撞中，照明塌暗、
噪声爆炸。（03 号场景深夜变体）
...
全部 15 个场景的成图、说明与渲染统计见 [docs/GALLERY.md](docs/GALLERY.md)；
...
`features.py` 全特性、`01…15` 画廊，其中 `06-spot-cascade`/`12-molten-oracle`/`15-assembly-hall` 需要 PhysX GPU；
网格资产全部入库，仅 `10/11/12/15` 的 HDR 天空由 `scripts/fetch-assets.sh` 下载）。
