...
- **旗舰场景 `16-atelier`「工作室」**（2560×1440 / 768 spp）：壁炉程序化
  火焰染暖烟黑炉膛与石台；彩色塑料积木（新自制资产 box/brick.obj 走
  PhysX mesh 凸包）、金银铜球、磨砂玻璃球与 Spot 全部从空中落下、由
  1.5 秒 PhysX 仿真算出落定姿态；多材质 Sparky 逐 usemtl 组独立刚体
  **落地散架**（用户确认的效果）；浅石盆 Beer–Lambert 清水、拉丝金属
  罐、顶部区域光 + 斜向聚光软阴影、低强度 HDR 天空擦金属/玻璃高光
- **旗舰场景 `17-dusk-tide`「暮潮观测站」**（2560×1440 / 768 spp）：
  日落后海岸平台——Spot 居检修台，Sparky 亮屏操作光学仪器（铬镜筒/
  玻璃物镜/涂层外壳/粗金属座），胶囊吉祥物校准格纹金属样片；材质
  点名逐项落实（陶瓷=白塑料低粗糙、粗糙金属、铬、低粗糙涂层、纹理
  表面）；地平线余晖亮带在前景 fbm 波浪水面拉出光道，与火焰信标暖
  反射、天空冷调交错（RGB Beer 暮海吸收）
- 自制网格资产 `assets/box.obj`/`brick.obj`（各 12 三角；动态刚体要求
  均匀缩放，砖比例内嵌网格）；README/GALLERY 旗舰板块扩为 15/16/17
  三张 2K 并列（render-gallery.sh HEROES 化）

### 近似替换清单（描述特性 → 渲染器现有机制，按用户授权自行替换）

- 多材质模型连体落体 → 逐 usemtl 组独立刚体：Sparky 部件贴合、散架
  温和；capsule_mascot 部件凸包深度穿插、动态化会被求解器爆炸分离，
  故 16 号中静置旁观（DESC 如实注明）
- "带法线纹理的金属样片" → 反照率格纹纹理金属（渲染器无 normal map）
- "低角度日落 HDR 天光" → gradient 暮色 + distant 低阳双灯 + 地平线
  余晖发光带（库内 HDRI 为正午晴空，太阳仰角不可调）
- "水面微表面 Fresnel" → water 的光滑菲涅尔界面 + fbm 法线扰动
  （water 固定光滑极限，见报告第 13 章）

红线记录：GB200 口径同 v0.16（16/17 号 2K 图与重渲的 15 号为
DEVARCH=compute_100 口径）；golden 基线零触碰，16/17 暂不进 golden
——口径债扩为 15/16/17 三场景（5090 后补钉 golden、补 BENCHMARKS
三行、按 5090 口径重渲复核；run-benchmark FEATURE_SCENES 已预先
加入三场景）；scenelib/渲染器代码零改动（"不修改渲染器"约束达成）。

（主要提交 c82dbb3 · 8070c8a）
