...
同 256 spp 的深夜变体（撤去太阳与补光，Sparky 的发光屏是唯一光源）：开——发光网格按三角形面积 CDF 被 NEE 主动采样，屏光照明干净；关——同样的发光网格只能被 BSDF 路径偶然撞中，照明塌暗、噪声爆炸。（03 号场景深夜变体 · 256 spp）
...
| 15-assembly-hall | 2560x1440 | 768 | 否 | 143.52 | 137 | 2334 |

> 口径注：15-assembly-hall 与 compare/ 对比图当前由 GB200（DEVARCH=compute_100）渲染，统计数字不与上表其余行（RTX 5090 + 驱动 615.36 口径）比较；待分配到 5090 测试机后统一重渲复核。
