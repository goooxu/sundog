...
顺序不可交换：曝光 $`2^{\text{exposure}}`$（以摄影的"档"为单位，加 1 档亮度翻倍，默认 0）必须乘在曲线**之前**——它的意义正是沿 S 曲线滑动工作点，先曲线后曝光就只是给成品图加亮；伽马是显示编码（1.1 节），永远在最后。`tonemap:"clamp"` 的线性退路则留给需要"所见即线性"的数值实验：第 14 章的白炉测试要从 PNG 直接读回线性能量、本章的 gamma 对比图要隔离伽马的效果，都必须绕过曲线。
