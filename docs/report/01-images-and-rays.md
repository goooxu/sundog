...
其中 $`m_1 = 2610/16384`$、$`m_2 = 2523/4096\times 128`$、$`c_1 = 3424/4096`$、$`c_2 = 2413/4096\times 32`$、$`c_3 = 2392/4096\times 32`$（对账 `pqOetf()`（src/transfer.h），逆变换 `pqEotf()` 同文件）。这条曲线按人眼的对比敏感度分配码值：0.05 到 10,000 尼特——六个数量级——被一条曲线覆盖，且任意两个相邻码值的亮度差都恰好贴着可觉察阈值走。几个锚点：100 尼特（SDR 峰值白）编码到 0.508，1,000 尼特到 0.752，10,000 尼特到 1.0——SDR 的整个世界只占了 PQ 码值域的一半，另一半全部留给高光。
