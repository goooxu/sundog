...
行进的成本控制靠**先粗筛再细算**——与[第 8 章](08-acceleration.md)包围盒剪枝同一思想的体积版。每团火焰配一个竖直包围圆柱（半径 $`R`$、高 $`H`$），raygen 用几行解析几何算出光线与圆柱的相交区间（y 平板与无限圆柱各裁一次，对账 `clipFlameBounds()`（device/volume.cuh）），没穿过包围柱的光线一步都不用走。火焰因此完全不进 OptiX 的加速结构与 SBT：它们存在 launch params 的一个小数组里，对 GAS/IAS/anyhit 零侵入。代价上这笔账很划算：07 场景 64 spp 一帧渲染 0.057 秒、5298 Mrays/s（docs/BENCHMARKS.md 特性层，含 §13.6 的阴影线行进）——与不含体积的场景同一量级，因为只有穿过火焰包围柱的那一小撮光线付了行进的钱；而无火焰的场景连分支都不进，golden 六场景 PSNR 全为 inf，逐位证明体积代码对既有渲染零扰动。
...
**抖动与决定性的账要重新对一遍。** 阴影行进和 §13.3 一样抖动起点（定相采样的带状伪影在大面积软半影上格外显眼，抖动化带为噪、随 spp 收敛），这意味着 NEE 每碰到一团火焰包围柱就多抽一次 PCG——渲染的随机流变了，这不就破坏决定性了吗？账分两层：其一，抽取契约与 `marchFlames` 相同——**裁剪早退之后才抽**，没穿过任何包围柱的阴影线一次都不抽，所以无火焰的场景随机流逐位照旧，golden 六场景 PSNR 维持 inf；其二，行进放在"BVH 已判定不遮挡"的早退分支之内（全黑的阴影线不值得行进），而某条固定线段是否被遮挡是场景的确定性性质、与 anyhit 触发顺序无关，条件抽取仍是（场景, 像素, 样本, 种子）的纯函数——同机同驱动双渲，含火焰的场景照样逐位一致。
