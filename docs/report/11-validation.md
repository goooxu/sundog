...
**第一层：场景解析单元测试。** 渲染之外的主机逻辑里，最容易悄悄出错的是场景装配：场景描述到几何/材质/灯的翻译、变换链的复合、面光源的世界空间面积与法线朝向。`test_scene_build`（tests/host/）在无 GPU 的情况下编译运行（只需 CUDA/OptiX 头文件），直接驱动渲染库的 C 场景构建 API（src/sundog_api.h）逐字段核对派生结果——变换合成、NEE 面光的世界空间几何、火焰双灯、物理约束——并验证十几类非法调用被明确拒绝、失败的调用不留副作用；Python 侧另有 `tests/test_scenelib.py` 钉住 scenelib 的调用流打包、校验层与决定性契约（含逐场景构建检查）。
