...
**第一层：场景解析单元测试。** 渲染之外的主机逻辑里，最容易悄悄出错的是场景装配：场景描述到几何/材质/灯的翻译、变换链的复合、面光源的世界空间面积与法线朝向。`test_scene_json`（tests/host/）在无 GPU 的情况下编译运行（只需 CUDA/OptiX 头文件 + python3），把每个 Python 场景经 `--emit-json` 发出的中间表示逐字段核对解析结果，并验证十几类非法输入被明确拒绝——错误场景要报人话，而不是渲出一张黑图；Python 侧另有 `tests/test_scenelib.py` 钉住 scenelib 的发出骨架、校验层与决定性契约。
