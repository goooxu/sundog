...
[第 13 章·体积渲染](13-volumes.md)之后，渲染器的"材质观"已经覆盖了表面与介质两个世界。本章讲一种恰好横跨两者的东西：画廊 08 号场景里那面既照出落日、又能看见水底、深处还泛着蓝绿的湖水。"水"在渲染里到底是什么？答案有点反高潮——**它没有任何新数学**，只是把第 5、6、13 章各自讲过的三件事拼成一种材质。本章把这三层拆开，再看它们的组合怎么产生波光、破碎倒影与水色这些一眼可辨的"水味"。
...
工程上这是 raygen 路径循环里的一个**介质状态**：局部量 `mediumAbsorb`（零向量 = 真空）。水面上的**透射**事件切换它——从正面折射进水置为材质的 $`\sigma`$、从背面折射出水清零；全内反射是反射，不切换。此后每次 `optixTrace` 返回，先按段长把 $`\beta \mathrel{*}= e^{-\sigma\, t}`$ 再处理命中（对账 raygen 的 mediumAbsorb 记账段（device/programs.cu））。开放水域里折射光可能在介质内 miss（射向无穷远），按大程长衰减到近零——无限深的水本就该吞掉一切。判断"是否透射"复用了现有的半空间检验 $`\langle \mathbf{w}_i, \mathbf{n}_s\rangle < 0`$，与 offsetRay 选边是同一逻辑，波纹扰动过的法线也自动适用。

## 14.4 工程记账与边界

如实声明三条边界。**嵌套介质不支持**：介质状态是单个变量而非栈，水里再放玻璃，玻璃内部会按真空算——08 号场景没有这种嵌套。**水下 NEE 死区**：阴影光线把透射材质当不透明是全项目的既有折衷（[附录](appendix-pitfalls.md)陷阱 3），水下着色点朝水上光源的阴影线因此必被水面挡住，水下照明只来自恰好折射穿出水面的 BSDF 路径——浅水没问题，深水底会偏暗，场景设计时让水底可见区域保持浅即可。**决定性不破**：波纹与吸收都是位置的纯函数，08 号场景双渲 PNG 逐位一致。

性能上水几乎免费：波纹是每次水面命中多四次 fbm 求值，吸收是每段一个指数。08 号场景 64 spp 一帧渲染 0.030 秒、7445 Mrays/s（docs/BENCHMARKS.md）——全画廊最快，因为开阔场景大半光线一跳就 miss 进天空。
...
水面 = 三件已学之事的组合：第 5 章的电介质界面换个折射率（1.33），第 6 章的梯度法线套上第 13 章的 fbm 高度场（只骗着色不动几何，掠射角回退防黑边），第 13 章的 Beer–Lambert 吸收由界面围出介质（透射切换状态、TIR 不切换、波长偏心的 $`\sigma`$ 给出水色）。波光路径、破碎倒影与深水蓝绿都不是新机制，而是这三层组合的自然涌现。正文至此收束——至于这些验证手段在开发中各自逮住过什么、那些一度让图"照样好看"的数值陷阱长什么样，请移步[附录·路径追踪常见实现陷阱](appendix-pitfalls.md)的案例复盘。
