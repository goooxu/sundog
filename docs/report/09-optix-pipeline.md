...
`main.cpp` 把前几章的所有部件串成一条直线：解析 CLI 并覆盖场景参数→加载场景（Python 场景文件先经 scenelib 发出 JSON 中间表示）→上传纹理→加载 OBJ 网格→只为**用到的** quadric 种类建 GAS、每个网格建 GAS、再建 IAS（第 8 章）→创建 `Pipeline`（编译模块、创建 1+2+8 个程序组、链接）并组装 SBT→分配胶片缓冲、填 `LaunchParams`→渲染循环→可选降噪→写 PNG 与 stats。渲染循环本体不过几行（src/main.cpp）：
