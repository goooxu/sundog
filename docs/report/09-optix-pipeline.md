...
**③ 编译程序**。十个设备程序全部写在唯一的模块 `device/programs.cu` 里，构建期由 nvcc 编成 PTX、`bin2c` 嵌入渲染库，运行期 `optixModuleCreate` 交给驱动即时编译——教科书路线是 OptiX-IR，sundog 因工程坑退回 PTX（见 9.6 节末段）。
...
**⑤ 调用 optixLaunch**。主机把 spp 按固定块大小（16）切块，循环发射并同步，每次 launch 覆盖全部像素、每像素一个线程（src/capi_render.cpp 渲染循环，代码见 9.6 节）。
...
**⑦ 降噪与落盘**。渲染循环结束后，OptiX Denoiser 在线性 HDR 域、以 albedo/normal 为引导层做 AI 降噪（`Denoiser`（src/denoise.cpp），原理见[第 10 章](10-sampling-denoising.md)），最后 PQ 编码写出 12-bit HDR AVIF（[第 1 章](01-images-and-rays.md)）。
...
| ⑤ 发射 | `optixLaunch` | src/capi_render.cpp 渲染循环 | 9.6 节 |
...
`sundog_render()`（src/capi_render.cpp）把前几章的所有部件串成一条直线：场景经 scenelib 的 ctypes 调用直灌进库（零中间表示）、CLI 覆盖由 scenelib 以选项结构传入→上传纹理→加载 OBJ 网格→只为**用到的** quadric 种类建 GAS、每个网格建 GAS、再建 IAS（第 8 章）→创建 `Pipeline`（编译模块、创建 1+2+8 个程序组、链接）并组装 SBT→分配胶片缓冲、填 `LaunchParams`→渲染循环→可选降噪→写 AVIF 与 stats。渲染循环本体不过几行（src/capi_render.cpp）：
