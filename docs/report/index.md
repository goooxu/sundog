...
- **完整阅读**（推荐，约 3-4 万字）：按 01 → 14 → 附录顺序。
- **只关心原理**：01 → 02 → 03 → 04 → 05（成像、渲染方程、蒙特卡洛、路径追踪、材质），可加 13 章 13.1–13.2（辐射传输）与 14 章（水面三层组合）。
- **只关心 GPU 工程**：01（速览）→ 09 章 9.1（OptiX 应用七步流程总览）→ 08 → 09 → 10 → 11 → 12 → 13 → 14（加速结构、OptiX、决定性、验证、物理装载、体积渲染、水面）。
...
| [11](11-validation.md) | 验证方法学与性能 | 怎么证明"算得对"？数千 Mrays/s 的吞吐与约 17 倍降噪等效收益从哪来？ |
