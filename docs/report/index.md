...
一路讲到"现代 NVIDIA GPU 上每秒数十亿次求交是怎么发生的"。数学假设读者具备
...
全书按主题分五组：**理论基础**（01–05：成像、渲染方程、蒙特卡洛、路径追踪、材质）、
**几何与工程**（06–09：求交、变换、加速结构、OptiX）、**质量**（10–11：采样降噪、
验证）、**光传输特性**（12–17：体积、水面、环境光、透明阴影、粗糙电介质、塑料）、
**场景机械**（18：物理装载——唯一不在光传输主线上的一章）。

- **完整阅读**（推荐，约 3-4 万字）：按 01 → 18 → 附录顺序。
- **只关心原理**：01 → 02 → 03 → 04 → 05（成像、渲染方程、蒙特卡洛、路径追踪、材质），可加 12 章 12.1–12.2（辐射传输）、13 章（水面三层组合）、14 章（环境光重要性采样——第 3 章逆变换采样的实战应用）、15 章（透明阴影与嵌套介质）、16 章（微表面透射——第 5 章 GGX 从反射到透射的收官）与 17 章（塑料——首个双瓣混合的采样记账）。
- **只关心 GPU 工程**：01（速览）→ 09 章 9.1（OptiX 应用七步流程总览）→ 08 → 09 → 10 → 11 → 12 → 13 → 14 → 15 → 16 → 17 → 18（加速结构、OptiX、决定性、验证、体积渲染、水面、环境光照、透明阴影与嵌套介质、粗糙电介质、塑料、物理装载）。
...
| [11](11-validation.md) | 验证方法学与性能 | 怎么证明"算得对"？数百到数千 Mrays/s 的吞吐与约 22 倍降噪等效收益从哪来？ |
| [12](12-volumes.md) | 体积渲染：程序化火焰 | 光穿过"会发光的空气"时发生了什么？一团火的形状从哪来？ |
| [13](13-water.md) | 水面：界面、波纹与水色 | 水为什么是三件已学之事的组合？波光路径从哪来？ |
| [14](14-envlight.md) | 环境光照：HDR 环境贴图与重要性采样 | 一张天空照片怎么变成可采样的光源？小太阳为什么让均匀采样爆炸？ |
| [15](15-transparent-media.md) | 透明阴影与嵌套介质 | 玻璃为什么曾投实心黑影？水中的玻璃怎么折射才对？ |
| [16](16-rough-dielectric.md) | 粗糙电介质：微表面透射与磨砂玻璃 | 磨砂玻璃为什么"透光不透形"？一个粗糙度怎么同时驱动反射与折射两个波瓣？ |
| [17](17-plastic.md) | 塑料：涂层与首个双瓣混合 BSDF | 有色底 + 白高光怎么共享一套采样账本？为什么 G/G₁ 化简到这里失效？ |
| [18](18-physics.md) | 物理装载：PhysX GPU 刚体模拟 | 512 只堆叠奶牛的位姿从哪来？物理怎么与渲染共卡且可复现？ |
