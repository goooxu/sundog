...
- **完整阅读**（推荐，约 3-4 万字）：按 01 → 16 → 附录顺序。
- **只关心原理**：01 → 02 → 03 → 04 → 05（成像、渲染方程、蒙特卡洛、路径追踪、材质），可加 13 章 13.1–13.2（辐射传输）、14 章（水面三层组合）、15 章（环境光重要性采样——第 3 章逆变换采样的实战应用）与 16 章（透明阴影与嵌套介质）。
- **只关心 GPU 工程**：01（速览）→ 09 章 9.1（OptiX 应用七步流程总览）→ 08 → 09 → 10 → 11 → 12 → 13 → 14 → 15 → 16（加速结构、OptiX、决定性、验证、物理装载、体积渲染、水面、环境光照、透明阴影与嵌套介质）。
