...
验证）、**光传输特性**（12–16：体积、水面、环境光、透明阴影、粗糙电介质）、
**场景机械**（17：物理装载——唯一不在光传输主线上的一章）。

- **完整阅读**（推荐，约 3-4 万字）：按 01 → 17 → 附录顺序。
- **只关心原理**：01 → 02 → 03 → 04 → 05（成像、渲染方程、蒙特卡洛、路径追踪、材质），可加 12 章 12.1–12.2（辐射传输）、13 章（水面三层组合）、14 章（环境光重要性采样——第 3 章逆变换采样的实战应用）、15 章（透明阴影与嵌套介质）与 16 章（微表面透射——第 5 章 GGX 从反射到透射的收官）。
- **只关心 GPU 工程**：01（速览）→ 09 章 9.1（OptiX 应用七步流程总览）→ 08 → 09 → 10 → 11 → 12 → 13 → 14 → 15 → 16 → 17（加速结构、OptiX、决定性、验证、体积渲染、水面、环境光照、透明阴影与嵌套介质、粗糙电介质、物理装载）。
...
| [17](17-physics.md) | 物理装载：PhysX GPU 刚体模拟 | 512 只堆叠奶牛的位姿从哪来？物理怎么与渲染共卡且可复现？ |
