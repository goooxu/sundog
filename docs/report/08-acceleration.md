...
回到 05 场景的实测数字（`out/05-check.stats.json`，960×540、32 spp；表格版见 `docs/BENCHMARKS.md` A 特性层）：

- **构建**：2 个 GAS（矩形 quadric + 奶牛网格）加 1 个 32770 实例的 IAS，共 2.1 ms；
- **渲染**：12.3 ms 追完 45 309 758 条光线，吞吐 3697 Mrays/s（`docs/BENCHMARKS.md` 中 64 spp 一档为 0.025 s、3719 Mrays/s；Mrays/s 的含义与解读见[第 11 章·验证方法学与性能](11-validation.md)）；
