...
[第 6 章·几何求交](06-geometry.md)解决了"光线怎么打中一个图元"，[第 7 章·变换与实例化](07-transforms.md)解决了"一份几何怎么摆出一万个物体"。本章回答规模问题：当场景里有 1.9 亿个三角形、一帧要追踪上亿条光线时，为什么整帧渲染只需要两百多毫秒？答案分两层——数据结构（层次包围盒）把每条光线的工作量从 O(N) 降到 O(log N)，硬件（RT Core）再把这套遍历与求交固化成专用电路。

## 8.1 朴素求交有多贵：先算一笔账

最直接的做法是：每条光线对场景里**每一个**图元做一次求交测试，取最近正根。对 N 个图元这是 O(N) 的工作量。N 很大时到底意味着什么？我们用本书最大的场景（`scenes/05-spot-swarm.py`，即画廊图 [05-spot-swarm](../gallery/05-spot-swarm.png)）算给读者看。
...
一次实测（分辨率 1920×1080，每像素采样数（samples per pixel/spp，定义见[第 1 章·成像与光线](01-images-and-rays.md)）为 128，数据存于 `out/gallery/05-spot-swarm.stats.json`，由 `scripts/render-gallery.sh` 生成）中，这一帧共追踪了 737 624 333 条光线（含弹射与阴影光线，约 $`7.38\times10^{8}`$ 条）。若按朴素算法，光线-三角形测试次数是

```math
7.38\times10^{8} \times 1.92\times10^{8} \approx 1.4\times10^{17} \text{ 次}.
```

一次光线-三角形求交（如常用的 Möller–Trumbore 算法：两次叉乘、四次点乘再加若干标量运算）约需 30 次浮点乘法，于是一帧的乘法总量约 $`4.2\times10^{18}`$ 次。即便假设有一台每秒能做 $`10^{14}`$ 次乘法（百 TFLOPS 量级）的理想机器全速只干这一件事，也要约 42000 秒——**近十二个小时一帧**。而实测这帧渲染只花了 230.9 毫秒：中间隔着约 $`2\times10^{5}`$ 倍的鸿沟。结论显而易见：绝大多数"光线×三角形"配对根本不能去测，必须有办法**批量排除**。
...
BVH 的效果好坏取决于构建质量：好的划分让兄弟盒体积小、重叠少，光线才更容易"只进一边"。OptiX 的构建器在驱动内部完成，具体算法不对用户暴露；反面教材是朴素实现常见的"随机选轴 + 中位数切分"——它只求两边图元数量均衡，不管划分出的兄弟盒有多大、重叠多少，剪枝效率因此大打折扣（见[附录](appendix-pitfalls.md)）。
...
RT Core 是加速器而非前提：OptiX 在没有 RT Core 的 GPU 上（如面向计算的数据中心 Blackwell，`nvidia-smi` 报 "RT core: 0"）自动改走 SM 上的软件 BVH 遍历，API、程序与渲染结果完全不变，只是"找最近命中"退回与着色争抢通用计算单元——速度打折，正确性与决定性分毫不差。sundog 因此不把 RT Core 列为运行要求，只要求 GPU 能跑 OptiX 与 PhysX 的 CUDA 内核（Turing 起）。
...
对账到代码（src/accel.cpp）：`buildQuadricGas()` 给每种**用到的**解析图元建一个只含单个 AABB 的 GAS（盒子来自 `quadricAabb()`（device/intersect.cuh），如抛物面是 $`[-1,1]\times[0,\tfrac12]\times[-1,1]`$，仅在 y 方向外加 $`10^{-4}`$ 的浮点余量，x、z 两向保持恰好 $`\pm 1`$），全场景同类物体共享；`buildTriangleGas()` 给每个 OBJ 网格建一个三角形 GAS；`buildIas()` 为每个场景对象填一条 `OptixInstance`——变换矩阵、`instanceId = i`、`sbtOffset = 2i`（这串偏移是第 9 章 SBT 的伏笔）以及是否 `DISABLE_ANYHIT` 的标志。
...
- **渲染**：230.9 ms 追完 737 624 333 条光线，吞吐 3194 Mrays/s（`docs/BENCHMARKS.md` 中 64 spp 一档为 0.115 s、3207 Mrays/s；Mrays/s 的含义与解读见[第 11 章·验证方法学与性能](11-validation.md)）；
- **显存**：整个进程峰值 708 MB——含帧缓冲、纹理与网格，而非几何复制的 GB 量级。

按吞吐折算，平均每条光线只摊到约 0.31 纳秒。这当然不是单条光线的延迟——单次显存访问都不止这个时间——而是数十万条光线在整卡上并行流水的摊销结果。与 8.1 节"理想机器也要近十二个小时"的朴素估算对照，BVH 的对数剪枝加上 RT Core 的硬件化，共同兑现了这约五个数量级的差距。
