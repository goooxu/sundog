...
[第 6 章·几何求交](06-geometry.md)解决了"光线怎么打中一个图元"，[第 7 章·变换与实例化](07-transforms.md)解决了"一份几何怎么摆出一万个物体"。本章回答规模问题：当场景里有 1.9 亿个三角形、一帧要追踪上亿条光线时，为什么整帧渲染只需要秒级（有 RT Core 的消费卡上更是数百毫秒）？答案分两层——数据结构（层次包围盒）把每条光线的工作量从 O(N) 降到 O(log N)，硬件（RT Core）再把这套遍历与求交固化成专用电路。

## 8.1 朴素求交有多贵：先算一笔账

最直接的做法是：每条光线对场景里**每一个**图元做一次求交测试，取最近正根。对 N 个图元这是 O(N) 的工作量。N 很大时到底意味着什么？我们用本书最大的场景（`scenes/05-spot-swarm.py`，即画廊图 [05-spot-swarm](../gallery/05-spot-swarm.avif)）算给读者看。
...
一次实测（分辨率 1920×1080，每像素采样数（samples per pixel/spp，定义见[第 1 章·成像与光线](01-images-and-rays.md)）为 128，数据存于 `out/gallery/05-spot-swarm.stats.json`，由 `scripts/render-gallery.sh` 生成）中，这一帧共追踪了 737 638 083 条光线（含弹射与阴影光线，约 $`7.38\times10^{8}`$ 条）。若按朴素算法，光线-三角形测试次数是

```math
7.38\times10^{8} \times 1.92\times10^{8} \approx 1.4\times10^{17} \text{ 次}.
```

一次光线-三角形求交（如常用的 Möller–Trumbore 算法：两次叉乘、四次点乘再加若干标量运算）约需 30 次浮点乘法，于是一帧的乘法总量约 $`4.2\times10^{18}`$ 次。即便假设有一台每秒能做 $`10^{14}`$ 次乘法（百 TFLOPS 量级）的理想机器全速只干这一件事，也要约 42000 秒——**近十二个小时一帧**。而实测这帧渲染只花了 2.17 秒（当前入库口径：GB200，无 RT Core，OptiX 软件遍历；带 RT Core 的 RTX 5090 曾实测同帧 230.9 毫秒）：即便按软遍历口径，中间也隔着约 $`2\times10^{4}`$ 倍的鸿沟。结论显而易见：绝大多数"光线×三角形"配对根本不能去测，必须有办法**批量排除**。
