...
一次实测（分辨率 1920×1080，每像素采样数（samples per pixel/spp，定义见[第 1 章·成像与光线](01-images-and-rays.md)）为 32，数据存于 `out/05-1080p-32.stats.json`）中，这一帧共追踪了 184 400 592 条光线（含弹射与阴影光线，约 $`1.84\times10^{8}`$ 条）。若按朴素算法，光线-三角形测试次数是

```math
1.84\times10^{8} \times 1.92\times10^{8} \approx 3.5\times10^{16} \text{ 次}.
```

一次光线-三角形求交（如常用的 Möller–Trumbore 算法：两次叉乘、四次点乘再加若干标量运算）约需 30 次浮点乘法，于是一帧的乘法总量约 $`1.1\times10^{18}`$ 次。即便假设有一台每秒能做 $`10^{14}`$ 次乘法（百 TFLOPS 量级）的理想机器全速只干这一件事，也要约 10600 秒——**近三个小时一帧**。而实测这帧渲染只花了 44.1 毫秒：中间隔着约 $`2\times10^{5}`$ 倍的鸿沟。结论显而易见：绝大多数"光线×三角形"配对根本不能去测，必须有办法**批量排除**。
...
回到 05 场景的实测数字（`out/05-1080p-32.stats.json`，1920×1080、32 spp；表格版见 `docs/BENCHMARKS.md` A 特性层）：

- **构建**：2 个 GAS（矩形 quadric + 奶牛网格）加 1 个 32770 实例的 IAS，共 2.1 ms；
- **渲染**：44.1 ms 追完 184 400 592 条光线，吞吐 4185 Mrays/s（`docs/BENCHMARKS.md` 中 64 spp 一档为 0.088 s、4189 Mrays/s；Mrays/s 的含义与解读见[第 11 章·验证方法学与性能](11-validation.md)）；
- **显存**：整个进程峰值 708 MB——含帧缓冲、纹理与网格，而非几何复制的 GB 量级。

按吞吐折算，平均每条光线只摊到约 0.24 纳秒。这当然不是单条光线的延迟——单次显存访问都不止这个时间——而是数十万条光线在整卡上并行流水的摊销结果。与 8.1 节"理想机器也要近三个小时"的朴素估算对照，BVH 的对数剪枝加上 RT Core 的硬件化，共同兑现了这约五个数量级的差距。
