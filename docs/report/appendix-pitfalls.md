...
**sundog 的做法**：把"布尔遮挡"升级为"直线透射率"——阴影线穿过玻璃/水时逐界面乘菲涅尔透过 $`(1-F)`$、介质段按 Beer–Lambert 衰减（`shadowAnyhit()`（device/programs.cu），机制详见[第 16 章·透明阴影与嵌套介质](16-transparent-media.md)），玻璃的影子由此有了透光甚至颜色。sundog 早期也曾如实采用上述折衷（`--opaque-shadows` 仍可复现旧行为），靠"主力光源是发光几何体、BSDF 折射路径 + MIS 兜底"缓解——那条缓解路线的局限（点光/平行光/环境光后方全黑、水下直接光全灭）正是升级的动因。注意透射率累积仍不折弯光线：**焦散**（曲面聚焦的亮斑）依旧只能靠 BSDF 折射路径撞出来，这一半的取舍仍然在场。
