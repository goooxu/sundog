...
**sundog 的做法**：把"布尔遮挡"升级为"直线透射率"——阴影线穿过玻璃/水时逐界面乘菲涅尔透过 $`(1-F)`$、介质段按 Beer–Lambert 衰减（`shadowAnyhit()`（device/programs.cu），机制详见[第 15 章·透明阴影与嵌套介质](15-transparent-media.md)），玻璃的影子由此有了透光甚至颜色。sundog 早期也曾如实采用上述折衷（`--opaque-shadows` 仍可复现旧行为），靠"主力光源是发光几何体、BSDF 折射路径 + MIS 兜底"缓解——那条缓解路线的局限（点光/平行光/环境光后方全黑、水下直接光全灭）正是升级的动因。注意透射率累积仍不折弯光线：**焦散**（曲面聚焦的亮斑）依旧只能靠 BSDF 折射路径撞出来，这一半的取舍仍然在场。
...
**sundog 的做法**：主机侧 `composeTransform()`（src/scene_build.cpp）把变换步列表折叠成一个 `Affine` 复合矩阵，实例变换由 OptiX 在遍历时应用（[第 7 章·变换与实例化](07-transforms.md)）——设备代码里没有任何逐光线的三角函数。
...
六组陷阱里，第 1、2 条是数学错误（有偏或错误的物理），第 3 条是朴素实现常见的近似（sundog 后来以直线透射率修复了直接光的一半，焦散的一半仍靠 BSDF 路径），第 4–6 条是工程质量问题（性能与可复现性）。它们共同的教训是：蒙特卡洛渲染器"图看着差不多"完全不构成正确性证据——分布错了、权重错了、判别式恒正了，图往往**仍然像那么回事**。这也是 sundog 把 golden 图像校验与逐位决定性（[第 11 章·验证方法学与性能](11-validation.md)）放进构建流程的原因。
