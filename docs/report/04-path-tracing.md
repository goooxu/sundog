...
2. **在灯上采样**：`sampleLight()`（device/light_sample.cuh）按灯型给出方向 `wi`、距离与立体角 pdf——矩形/圆盘在面上均匀取点后按第 3 章换元 `pdf = d2/(cosL*area)`；球灯在可见球冠内立体角均匀，`pdf = 1/(2π(1−cosθ_max))`，其中 $`\theta_{max}`$ 是球面对着色点张开的锥半角（$`\sin\theta_{max} = R/d`$，$`R`$ 为球半径，$`d`$ 为着色点到球心的距离）；三角网格灯先按面积 CDF 选三角形、再在三角形内均匀取重心（两级采样的完整推导见本节末尾）；点光/平行光是 delta 灯，标记 `isDelta`、`pdf = 1` 且衰减已折进 `Li`（点光实际在半径为 `radius` 的小圆盘上取点以产生软阴影，但记账上仍视为 delta 灯）；
3. **阴影线（shadow ray）验可见性**：起点 `offsetRay(x, ±ns)`（透射连线时翻到界面背侧，见[第 16 章·粗糙电介质](16-rough-dielectric.md)）防自交，`tmax = dist*0.999` 防打中灯本身；不透明表面算完全遮挡，穿透面与 alpha 镂空由 anyhit 放行，玻璃/水则沿直线累积透射率而非硬遮挡（机制见[第 15 章·透明阴影与嵌套介质](15-transparent-media.md)）；
4. **记账**：可见则累加
...
其中 $`\beta`$ 是 4.1 节的路径吞吐量，$`w`$ 是下一节将引入的 MIS 权重——本节可暂视为 1（对 delta 灯它恒为 1）。对应代码 `c = beta * f * fabsf(cosS) * ls.Li * w / pdfLe`（透射连线时 `cosS` 为负，故取绝对值），其中 `pdfLe = (ls.isDelta ? 1 : ls.pdf) / numLights`——选灯概率作为分母的一部分，等价于对"所有灯的贡献之和"做单样本估计。
...
### 三角网格光：面积 CDF、重心采样与两侧同口径

矩形/圆盘/球都有解析的均匀采样，可发光体清单里还差一类：**三角网格**。它曾经缺席——发光网格只能靠 BSDF 光线偶然撞中，而本节开头的方差论证在这里原样应验：一块小小的发光屏幕正是"小灯"的教科书案例，撞中概率低、$`L_e`$ 大，画面上全是萤火虫。补上它，本质上是回答一个问题：**怎么在一堆三角形上均匀采样一个点，并给出正确的立体角 pdf？**

**两级采样。** 目标分布是"按表面积均匀"：面积大的三角形应按比例更常被选中。第一级选三角形——把 $`N`$ 个三角形的**世界空间**面积做前缀和并归一化成 CDF（$`\text{cdf}[0]=0`$、$`\text{cdf}[N]=1`$），抽 $`u \sim U[0,1)`$ 后二分查找它落在哪个区间，即第 3 章逆变换采样的离散版；零面积的退化三角形区间为空，永远不会被选中。CDF 在 host 端构建：网格文件加载后把对象空间顶点乘上物体变换（非均匀缩放会不均匀地改变各三角形的面积比，所以必须在**世界空间**算面积），前缀和用 double 累加——上千个小三角形的浮点小数逐个相加，float 的舍入会让 CDF 尾部失真（对账 mesh 光 CDF 构建段（src/capi_render.cpp））。第二级在选中的三角形内均匀取点，用重心坐标的 $`\sqrt{}`$ 变换：

```math
b_1 = \sqrt{u_1}\,(1 - u_2),\qquad b_2 = \sqrt{u_1}\,u_2,\qquad
p = (1 - b_1 - b_2)\,p_0 + b_1\,p_1 + b_2\,p_2.
```

直觉上 $`\sqrt{u_1}`$ 是在"到顶点 $`p_0`$ 的相对距离"上采样——固定该距离的等值线是一条平行于对边的线段，长度正比于距离，均匀的面积测度要求距离的 pdf 线性增长，其 CDF 是二次函数，逆变换正是开方（与第 3 章圆盘采样的 $`r=\sqrt{u}`$ 同一个论证）。

**pdf 的换元。** 面积均匀意味着 $`p_A = 1/A_{\text{total}}`$（注意分母是**整个网格**的总面积——第一级按面积选三角形、第二级在三角形内均匀，两级复合后恰好整体均匀），换到立体角与矩形/圆盘同一条公式：

```math
p_\omega = \frac{d^2}{\cos\theta_g\,A_{\text{total}}},
```

其中 $`\theta_g`$ 用被采样三角形的**几何法线**度量——面积到立体角的雅可比是几何量，与平滑着色无关（对账 `sampleLight()` 的 LT_MESH 分支（device/light_sample.cuh））。

**两侧同口径：一个换法线的技巧。** MIS 那侧的账（4.3 节）要求 BSDF 光线命中发光网格时，`lightPdfSolidAngle()` 给出与上式一致的 pdf——分母里的 $`\cos\theta_g`$ 需要**命中三角形的几何法线**。麻烦在于 payload 里装的是平滑插值的着色法线，几何法线不在其中，八个 payload 寄存器也早已用满。解法利用发光体的一条性质：**发光命中即终止路径**（raygen 记完账就 `break`，法线不再参与任何散射）——所以 closest-hit 在"命中的这一面是已登记的发光材质"时，直接把几何法线装进 payload 顶替着色法线（对账 `triShadePoint()`（device/programs.cu））。判断按**命中面解析后的材质**做门控：发光正面配 lambert 背面的单面灯，背面命中仍拿平滑法线继续弹跳。这个技巧的代价也如实入账：发光网格作为像素**首命中**时，法线 AOV 记下的是几何法线而非平滑法线——发光体是画面里最亮、最不需要降噪引导的像素，可以接受。还有一处镜像陷阱与矩形灯的 `detSign` 同源：采样侧的几何法线来自世界空间边向量的叉积，命中侧来自逆转置变换——镜像变换（$`\det M < 0`$）下两者符号相反，采样侧用预存的 $`\text{sign}(\det M)`$ 翻正（对账 `mNgSign` 字段（device/params.h））。

**纹理与单双面：老规矩新对象。** 上文"两头一致"的原则在网格上落实为：采样侧的 uv 用**同一份顶点 uv 缓冲、同一组重心权重**插值，与 closest-hit 的插值表达式逐位相同；网格没有 uv 时两侧都退回重心坐标本身。所以纹理化的发光网格可以放心当 NEE 灯（球面灯的"纹理化必须 nee=False"限制源自其 uv 框架在锥采样下不可用，不适用于网格）。单面判断同样用被采样三角形的几何法线，与命中侧由几何法线导出的 `frontface` 天然同侧。
...
- **NEE 侧**：`w = pdfLe / (pdfLe + pdfB)`，其中 `pdfB = bsdfPdf(mat, -d, ls.wi, ns, etaRel)`（末位的相对折射率供[第 16 章·粗糙电介质](16-rough-dielectric.md)的透射瓣使用，其他材质忽略）；
- **发光体命中侧**：BSDF 光线打中注册为灯的发光体时，`w = prevPdf / (prevPdf + pdfL)`，其中 `prevPdf` 是上一顶点 BSDF 采样的立体角 pdf，`pdfL = lightPdfSolidAngle(lights[lightId], prevX, x, nHit) / numLights`，`nHit` 即 payload 里的命中法线（网格灯处正是 4.2 节换入的几何法线）。注意这笔账记在**下一次**循环迭代：采样方向时把 `bs.pdf` 与当前顶点位置存入 `prevPdf`/`prevX`，命中发光体时才拿出来配权（对应 4.6 伪代码中 `prevPdf`/`prevX` 的赋值行）。
...
对账：raygen 里四处贡献（火焰体积发光、背景、发光体命中、NEE，火焰行进段那一处见[第 12 章·体积渲染](12-volumes.md)）累加前都有同一句
...
*图：04-parabolica 24 spp。左：`--clamp 0`（关闭），镜面/折射路径产生大量 firefly；右：`--clamp 5`（刻意取小值使抑制在低 spp 下肉眼可见；该场景默认 clamp 30 已能拦住最离谱的离群样本，与关闭的差异在低 spp 下很不显眼），亮斑消失，间接高光略有压暗。*
    ...
    if mat 是发光体:                                  # —— BSDF 命中灯，MIS 一侧
        if hit.frontface 或 mat.twoSided:             # 单面灯背面不发光，与 sampleLight 的背面拒绝一致
            ...
            if !specular 且 hit.lightId ≥ 0:
                pdfL ← lightPdfSolidAngle(灯, prevX, x, n) / numLights
                w ← prevPdf / (prevPdf + pdfL)
            L += clamp?(β · Le · w)
        break
    if 有灯 且 mat 非 delta:                          # —— NEE，MIS 另一侧
        k ← 均匀选一盏灯;  ls ← sampleLight(lights[k], x)
        if ls.valid 且 (cosS = ls.wi·n > 0 或 (cosS < 0 且 mat 为 dielectric)) 且 阴影线透射率 Tr > 0:
                                                          # 贡献 ×= Tr（第 15 章）；cosS < 0 为透射 NEE（第 16 章）
...
三种记账出口（背景、发光体、NEE）加一个延伸动作，就是路径追踪的全部骨架——[第 12 章·体积渲染](12-volumes.md)的火焰行进还会补上第四个记账出口（体积发光）。
