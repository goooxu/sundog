...
2. **在灯上采样**：`sampleLight()`（device/light_sample.cuh）按灯型给出方向 `wi`、距离与立体角 pdf——矩形/圆盘在面上均匀取点后按第 3 章换元 `pdf = d2/(cosL*area)`；球灯在可见球冠内立体角均匀，`pdf = 1/(2π(1−cosθ_max))`，其中 $`\theta_{max}`$ 是球面对着色点张开的锥半角（$`\sin\theta_{max} = R/d`$，$`R`$ 为球半径，$`d`$ 为着色点到球心的距离）；点光/平行光是 delta 灯，标记 `isDelta`、`pdf = 1` 且衰减已折进 `Li`（物理模式的点光实际在半径为 `radius` 的小圆盘上取点以产生软阴影，但记账上仍视为 delta 灯）；
3. **阴影线（shadow ray）验可见性**：起点 `offsetRay(x, ns)` 防自交，`tmax = dist*0.999` 防打中灯本身；途中任何实体表面都算遮挡（穿透面与 alpha 镂空由 anyhit 放行，见第 9 章）；
4. **记账**：可见则累加
...
对账 RR 段（device/programs.cu）：`depth >= 4` 才启动（前四段承载绝大部分能量，不值得赌），且 parity 模式（复刻 CPU 原版采样的对照模式，见 4.6 节与[第 11 章](11-validation.md)）未启用；存活率取吞吐量的最大分量并夹到区间内：
...
以下伪代码与 `__raygen__render()`（device/programs.cu）的物理模式逐行对应：
...
for depth = 0 .. maxDepth-1:
    ...
    if 有灯 且 mat 非 delta:                          # —— NEE，MIS 另一侧
        ...
        if ls.valid 且 cosS = ls.wi·n > 0 且 阴影线未被挡:
...
三种记账出口（背景、发光体、NEE）加一个延伸动作，就是路径追踪的全部骨架。parity 模式（复刻 CPU 原版采样以做公平对比，见[第 11 章·验证方法学与性能](11-validation.md)）会关闭 MIS 与 RR、并把 NEE 换成原版的直接光公式，此处不展开。
