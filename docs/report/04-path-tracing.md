...
3. **阴影线（shadow ray）验可见性**：起点 `offsetRay(x, ±ns)`（透射连线时翻到界面背侧，见[第 17 章·粗糙电介质](17-rough-dielectric.md)）防自交，`tmax = dist*0.999` 防打中灯本身；不透明表面算完全遮挡，穿透面与 alpha 镂空由 anyhit 放行，玻璃/水则沿直线累积透射率而非硬遮挡（机制见[第 16 章·透明阴影与嵌套介质](16-transparent-media.md)）；
4. **记账**：可见则累加
...
- **NEE 侧**：`w = pdfLe / (pdfLe + pdfB)`，其中 `pdfB = bsdfPdf(mat, -d, ls.wi, ns, etaRel)`（末位的相对折射率供[第 17 章·粗糙电介质](17-rough-dielectric.md)的透射瓣使用，其他材质忽略）；
...
对账：raygen 里四处贡献（火焰体积发光、背景、发光体命中、NEE，火焰行进段那一处见[第 13 章·体积渲染](13-volumes.md)）累加前都有同一句
...
for depth = 0 .. maxDepth-1:
    ...
    if 有灯 且 mat 非 delta:                          # —— NEE，MIS 另一侧
        ...
        if ls.valid 且 (cosS = ls.wi·n > 0 或 (cosS < 0 且 mat 为 dielectric)) 且 阴影线透射率 Tr > 0:
                                                          # 贡献 ×= Tr（第 16 章）；cosS < 0 为透射 NEE（第 17 章）
...
三种记账出口（背景、发光体、NEE）加一个延伸动作，就是路径追踪的全部骨架——[第 13 章·体积渲染](13-volumes.md)的火焰行进还会补上第四个记账出口（体积发光）。
